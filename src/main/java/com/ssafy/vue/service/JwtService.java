...
public class JwtService {
	...
	private Long expireMin = 30L;//  30*60*1000  => 5분  token 유효 시간 설정 
...
//		Payload 설정: client에게 줘야 하는 토큰 
		jwtBuilder
			.setSubject("로그인토큰") // 토큰의 제목 설정
			.setExpiration(new Date(System.currentTimeMillis() + 1000 * 60 * expireMin)) // 30분으로 유효기간 설정
			
			.claim("user", member).claim("greeting", "환영합니다. " + member.getUsername()); // 담고 싶은 정보 설정.
		
//		signature 설정 
//		SignatureAlgorithm.HS256 암호화를 기본으로 signature 설정  
		jwtBuilder.signWith(SignatureAlgorithm.HS256, signature.getBytes());
		
//		마지막 직렬화 처리        http://jwt.io
